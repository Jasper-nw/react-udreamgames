...
import { getGameLabel, getGameMenu } from "@/api";
...
export async function getStaticPaths() {
  const response = await fetch(
    ...
    }api/label/all?origin=udreamgames`,
    { method: "GET" }
  );
  const data = await response.json();
  const paths = data.data.map((item: any) => ({
    params: {
      title: `${item.name.toLowerCase()}-${item.id}`,
    },
  }));
...
export async function getStaticProps({
  ...
  const res = await getGameLabel({
    label: params.title.replace(
      /^.*?(\d*)$/,
      (str, match, index) => match || ""
    ),
    ...search,
  });
...
export default function LabelPage({
  ...
  const showMoreGame = () => {
    ...
    getGameLabel({
      ...search,
      label: title.replace(/^.*?(\d*)$/, (str, match, index) => match || ""),
    })
      .then((res) => {
        res.data.list &&
        ...
        <div className={styles["title-top"]}>
          ...
          <TextBg className={styles["title-content"]}>
            <div className={styles["title"]}>
              <span>{title.toUpperCase().replace(/-(\S*)/, "")}</span>
            </div>
          </TextBg>
        </div>
